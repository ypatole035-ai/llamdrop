...
try:
    from config import get_system_prompt, get_max_tokens, get_temperature
except ImportError:
...
def run_chat(cmd, model_name, device_profile, model_path=None, prompt_format='chatml',
    ...
    if BATTERY_AVAILABLE:
        ...
        warn_below = cfg_get("warn_battery_below", 15)
    ...
    try:
        while True:
            ...
            raw_out = _launch_llama(cmd, prompt, max_tokens, temperature,
...
def _clean(content):
    """Replace session placeholder with real text."""
...
def _launch_llama(cmd, prompt, max_tokens=300, temperature=0.7,
    ...
    try:
        ...
        # Stop thinking indicator before printing response
        ...
        # Find response start — differs by prompt format
        for marker in [
            "<|im_start|>assistant",           # chatml
            "<|start_header_id|>assistant<|end_header_id|>\n\n",  # llama3
            "<start_of_turn>model\n",          # gemma (last occurrence)
            "<|assistant|>\n",                 # phi3
