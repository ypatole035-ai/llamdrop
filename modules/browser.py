...
def filter_models_for_device(models, device_profile):
    ...
    for model in models:
        ...
        # Prefer Q4 if it fits, fall back to Q2
        for quant_pref in ["Q4_K_M", "Q5_K_M", "Q3_K_M", "Q2_K"]:
...
def draw_header(stdscr, device_profile, width):
    ...
    specs = f" RAM: {avail}GB free / {total}GB  |  {chip} "
...
def run_browser(stdscr, models, device_profile):
    ...
    while True:
        ...
        if key == curses.KEY_UP and selected > 0:
            ...
            return models[selected] if models else None
